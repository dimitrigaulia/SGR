...
import { TenantPerfilService, CreateTenantPerfilRequest, UpdateTenantPerfilRequest } from '../../../../features/tenant-perfis/services/perfil.service';

@Component({
  standalone: true,
  selector: 'app-tenant-perfil-form',
...
export class TenantPerfilFormComponent {
  private route = inject(ActivatedRoute);
  private router = inject(Router);
  private service = inject(TenantPerfilService);
  ...
  save() {
    ...
    const v = this.model;
    if (this.id() === null) {
      const req: CreateTenantPerfilRequest = { nome: v.nome, isAtivo: !!v.isAtivo };
      this.service.create(req)
        .pipe(takeUntilDestroyed(this.destroyRef))
        .subscribe({
          next: () => { this.toast.success('Perfil criado'); this.router.navigate(['/tenant/perfis']); },
          error: err => { 
      ...
      const req: UpdateTenantPerfilRequest = { nome: v.nome, isAtivo: !!v.isAtivo };
      this.service.update(this.id()!, req)
        .pipe(takeUntilDestroyed(this.destroyRef))
        .subscribe({
          next: () => { this.toast.success('Perfil atualizado'); this.router.navigate(['/tenant/perfis']); },
          error: err => { 
            const msg = err.error?.message || 'Erro ao salvar perfil'; 
            this.toast.error(msg); 
            this.error.set(msg);
