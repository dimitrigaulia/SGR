...
@Component({
  standalone: true,
  selector: 'app-tenant-canais-venda-list',
  ...
  templateUrl: './canais-venda-list.component.html',
  styleUrls: ['./canais-venda-list.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class TenantCanaisVendaListComponent implements OnDestroy {
  ...
  displayedColumns = ['nome', 'taxaPadrao', 'ativo', 'acoes'];
  data = signal<CanalVendaDto[]>([]);
  total = signal(0);
  pageIndex = signal(0);
  pageSize = signal(10);
  ...
  load() {
    ...
    this.service.list({ page, pageSize, sort, order, search })
      ...
      .subscribe({
        ...
        error: () => {
          this.toast.error('Falha ao carregar canais de venda');
          this.isLoading.set(false);
          this.cdr.markForCheck();
        }
      });
  ...
  delete(id: number) {
    this.confirmationService.confirmDelete('este canal de venda')
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(confirmed => {
        if (!confirmed) return;
        ...
        this.service.delete(id)
          ...
          .subscribe({ 
            next: () => { 
              this.toast.success('Canal de venda excluído'); 
              this.load();
            }, 
            error: (e: any) => {
              this.toast.error(e.error?.message || 'Falha ao excluir canal de venda');
              this.isLoading.set(false);
              this.cdr.markForCheck();
            }
          });
  ...
  edit(e: CanalVendaDto) {
    this.router.navigate(['/tenant/canais-venda/cadastro'], { state: { id: e.id } });
  }

  view(e: CanalVendaDto) {
    this.router.navigate(['/tenant/canais-venda/cadastro'], { state: { id: e.id, view: true } });
  }
