...
@Component({
  standalone: true,
  selector: 'app-tenant-insumos-list',
...
export class TenantInsumosListComponent implements OnDestroy {
  ...
  load() {
    ...
    this.service.list({ page, pageSize, sort, order, search })
      ...
      .subscribe({
        ...
        error: () => {
          this.toast.error('Falha ao carregar insumos');
          this.isLoading.set(false);
          this.cdr.markForCheck();
        }
      });
  ...
  delete(id: number) {
    this.confirmationService.confirmDelete('este insumo')
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(confirmed => {
        if (!confirmed) return;
        ...
        this.service.delete(id)
          ...
          .subscribe({ 
            ...
            error: (e: any) => {
              this.toast.error(e.error?.message || 'Falha ao excluir insumo');
              this.isLoading.set(false);
              this.cdr.markForCheck();
            }
          });
  ...
  edit(e: InsumoDto) {
    this.router.navigate(['/tenant/insumos/cadastro'], { state: { id: e.id } });
  }

  view(e: InsumoDto) {
    this.router.navigate(['/tenant/insumos/cadastro'], { state: { id: e.id, view: true } });
  }
