...
@Component({
  standalone: true,
  selector: 'app-tenant-unidades-medida-list',
  ...
  styleUrls: ['./unidades-medida-list.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class TenantUnidadesMedidaListComponent implements OnDestroy {
  ...
  load() {
    ...
    this.service.list({ page, pageSize, sort, order, search })
      ...
      .subscribe({
        ...
        error: () => {
          this.toast.error('Falha ao carregar unidades de medida');
          this.isLoading.set(false);
          this.cdr.markForCheck();
        }
      });
  ...
  delete(id: number) {
    this.confirmationService.confirmDelete('esta unidade de medida')
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(confirmed => {
        if (!confirmed) return;
        ...
        this.service.delete(id)
          ...
          .subscribe({ 
            ...
            error: (e: any) => {
              this.toast.error(e.error?.message || 'Falha ao excluir unidade de medida');
              this.isLoading.set(false);
              this.cdr.markForCheck();
            }
          });
  ...
  edit(e: UnidadeMedidaDto) {
    this.router.navigate(['/tenant/unidades-medida/cadastro'], { state: { id: e.id } });
  }

  view(e: UnidadeMedidaDto) {
    this.router.navigate(['/tenant/unidades-medida/cadastro'], { state: { id: e.id, view: true } });
  }
