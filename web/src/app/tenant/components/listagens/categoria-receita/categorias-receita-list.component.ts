...
@Component({
  standalone: true,
  selector: 'app-tenant-categorias-receita-list',
  ...
  templateUrl: './categorias-receita-list.component.html',
  styleUrls: ['./categorias-receita-list.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class TenantCategoriasReceitaListComponent implements OnDestroy {
  ...
  data = signal<CategoriaReceitaDto[]>([]);
  total = signal(0);
  pageIndex = signal(0);
  pageSize = signal(10);
  ...
  delete(id: number) {
    this.confirmationService.confirmDelete('esta categoria')
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(confirmed => {
        if (!confirmed) return;
        ...
        this.service.delete(id)
          ...
          .subscribe({ 
            ...
            error: (e: any) => {
              this.toast.error(e.error?.message || 'Falha ao excluir categoria');
              this.isLoading.set(false);
              this.cdr.markForCheck();
            }
          });
  ...
  edit(e: CategoriaReceitaDto) {
    this.router.navigate(['/tenant/categorias-receita/cadastro'], { state: { id: e.id } });
  }

  view(e: CategoriaReceitaDto) {
    this.router.navigate(['/tenant/categorias-receita/cadastro'], { state: { id: e.id, view: true } });
  }
